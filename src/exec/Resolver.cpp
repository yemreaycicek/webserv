 ...
 * @ Modified by: akosaca
 * @ Modified time: 2026-09-02 / 13:55:50
 */
...
namespace exec {
    ...
    const config::LocationBlock* Resolver::matchLocation(const config::ServerBlock& sb, const std::string& uri) const {
        ...
        for (std::vector<config::LocationBlock>::const_iterator it = sb.locations.begin(); it != sb.locations.end(); ++it){
            ...
            if (uri.compare(0, normPath.length(), normPath) == 0) {
                if ((normPath == "/" || uri.length() == normPath.length() || uri[normPath.length()] == '/') && max < normPath.length()) {
                    max = normPath.length();
                    res = &(*it);
                }
            }
    ...
    std::string Resolver::buildFsPath(const config::LocationBlock& loc, const std::string& uri) const {
        ...
        if (!remainder.empty() && !root.empty()) {
            if (remainder[0] != '/' && root[root.size() - 1] != '/') remainder = "/" + remainder;
            else if (remainder[0] == '/' && root[root.size() - 1] == '/') root.erase(root.size() - 1);
        }
